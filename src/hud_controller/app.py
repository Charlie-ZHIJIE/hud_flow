#!/usr/bin/env python3
"""
MCP Server application for MAC RX Stream problem evaluation.
"""
...
# MCP server name
mcp = FastMCP("mac_rx_evaluation", log_level="DEBUG", debug=True)
...
async def grade_problem(
    ...
    if attempt_num == 1:
        ...
        logger.info(f"Attempt #{attempt_num}: {reward:.3f} ({result.get('passed', 0)}/{result.get('total', 0)} tests) | First: {first_reward:.3f} | Delta: {improvement:+.3f}")
    ...
    if reward >= 1.0:
        message = f"SUCCESS! All {total} tests passed! Reward: {reward:.3f} (100%)"
    else:
        failed_list = ", ".join(failed_tests) if failed_tests else "(unknown)"
        message = f"TESTS FAILED: {passed}/{total} passed ({reward*100:.0f}%). Failed: {failed_list}. Read DEBUG_REPORT.md for details."
...
async def debug_test_results(
    ...
    if session_key not in _grading_attempts or not _grading_attempts[session_key]:
        return f"No grading attempts found. Call grade_problem(problem_id=\"{problem_id}\") first."
    
    last_attempt = _grading_attempts[session_key][-1]
    test_output = _last_test_output.get(session_key, '')
    
    if last_attempt['reward'] >= 1.0:
        return f"All tests passed! {last_attempt['passed']}/{last_attempt['total']} tests."
